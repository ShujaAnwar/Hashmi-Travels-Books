  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 sm:p-8 no-print flex flex-col items-center">
      <div className="w-full max-w-5xl flex justify-between items-center mb-6">
        ...
        <button onClick={handlePrint} className="bg-sky-700 hover:bg-sky-800 text-white px-8 py-2 rounded-lg font-bold flex items-center gap-2 shadow-lg transition-all">
      ...
      <div className="bg-white w-full max-w-5xl shadow-2xl p-12 print:shadow-none print:p-0 min-h-[11in] font-sans text-slate-800">
        <div className="flex justify-between items-start border-b border-slate-100 pb-8 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-slate-900 leading-none">{settings.appName.toUpperCase()}</h1>
              <p className="text-xs font-bold text-slate-400 tracking-[0.2em] mt-1">{settings.tagline.toUpperCase()}</p>
            </div>
          </div>
          <div className="text-right">
            <h2 className="text-xl font-bold text-sky-800">Hotel Booking Voucher</h2>
            <p className="text-rose-600 font-bold mt-1">{settings.corporateName}</p>
            <div className="text-xs text-slate-500 mt-4 leading-relaxed font-medium">
              <p>{settings.address}</p>
              <p>Cell: {settings.cell}</p>
        ...
        <div className="mt-8">
          ...
          <ul className="space-y-3 text-[11px] text-slate-600 leading-relaxed font-medium">
            ...
            <li className="flex gap-3">
              <span className="text-sky-500 font-bold">•</span>
              Rooms may not be available for early check-in, unless especially required in advance.
            </li>
            <li className="flex gap-3">
              <span className="text-sky-500 font-bold">•</span>
        ...
        <div className="mt-auto pt-8 border-t border-slate-100 text-[10px] text-slate-400 font-medium italic">
          <p>This is a computer generated document. Issued by {settings.corporateName}.</p>
        </div>
      </div>
    </div>
  );
