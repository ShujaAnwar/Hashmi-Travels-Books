  ...
  return (
    <div className="min-h-screen bg-slate-100 p-8 no-print flex flex-col items-center">
      <div className="w-full max-w-6xl flex justify-between items-center mb-6">
        ...
        <button onClick={() => window.print()} className="bg-sky-700 text-white px-8 py-2 rounded-lg font-bold flex items-center gap-2 shadow-lg hover:scale-105"><Printer size={18} /> Print Invoice</button>
      </div>
      ...
      <div className="bg-white w-full max-w-6xl p-12 print:p-0 font-sans text-slate-800 shadow-xl border border-slate-200 print:shadow-none print:border-none min-h-[11in]">
        <div className="flex justify-between mb-8">
           <div className="flex items-center gap-4">
              {settings.logoBase64 ? (
                 <img src={settings.logoBase64} className="w-20 h-20 object-contain" alt="Logo" />
              ) : (
                 <div className="bg-blue-600 p-2 rounded-lg text-white font-black">LOGO</div>
              )}
              <div>
                <h1 className="text-2xl font-black">{settings.appName.toUpperCase()}</h1>
                <p className="text-[10px] tracking-widest font-bold text-slate-400">{settings.tagline.toUpperCase()}</p>
              </div>
           </div>
           <div className="text-center">
              <h2 className="text-xl font-bold text-rose-600">{settings.corporateName}</h2>
           </div>
        ...
        <div className="text-[11px] mb-8 space-y-1 font-medium">
          ...
          <p><span className="font-bold">CELL :</span> {settings.cell} - <span className="font-bold">PHONE :</span> {settings.phone} - <span className="font-bold">EMAIL :</span> {settings.email}</p>
        ...
        <div className="mt-8 text-[11px] font-bold">
          ...
          <p className="text-rose-600 font-black uppercase text-sm">{settings.corporateName}</p>
        ...
        <div className="mt-auto pt-20 flex justify-between items-end border-t border-slate-50 text-[8px] text-slate-300 font-bold uppercase tracking-[0.3em]">
          <p>Developed by {settings.appName} System</p>
