...
import { Search, Filter, Printer, Eye, ExternalLink, Copy, Edit3, MoreHorizontal } from 'lucide-react';
...
const HotelVoucherList: React.FC<{ 
  ...
  return (
    <div className={`${isCompact ? 'space-y-3' : 'space-y-6'}`}>
      {/* Responsive Filters */}
      <div className={`bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row items-stretch lg:items-end gap-4 ${isCompact ? 'p-4' : 'p-5 lg:p-7'}`}>
        <div className="flex-1">
          <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Search Database</label>
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
            <input 
              type="text" 
              placeholder="Passenger or Hotel..." 
        ...
        <button className={`bg-slate-900 dark:bg-emerald-600 text-white rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 hover:opacity-90 transition-all ${isCompact ? 'px-4 py-2' : 'px-6 py-3.5'}`}>
          <Filter size={14} /> <span className="lg:hidden">Apply</span>
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-slate-900 dark:bg-slate-800 text-white uppercase text-[9px] font-black tracking-widest">
                ...
                <th className={`px-6 ${isCompact ? 'py-3' : 'py-5'}`}>Guest & Hotel</th>
              ...
              {filteredVouchers.map((v) => {
                ...
                return (
                  <tr key={v.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className={`px-6 ${isCompact ? 'py-2' : 'py-4'}`}>
                      <div className={`font-black uppercase text-slate-800 dark:text-slate-200 truncate max-w-[200px] ${isCompact ? 'text-[10px]' : 'text-xs'}`}>{v.pax_name} @ {v.hotel_name}</div>
                      <div className={`text-slate-500 font-bold uppercase tracking-wide mt-0.5 ${isCompact ? 'text-[8px]' : 'text-[9px]'}`}>{v.check_in} ({v.nights}N)</div>
                    </td>
                    ...
                    <td className={`px-6 ${isCompact ? 'py-2' : 'py-4'}`}>
                      <div className="flex gap-1">
                        <button onClick={() => onView(v.id, 'BOOKING')} className="p-1.5 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-500 rounded-lg hover:bg-amber-600 hover:text-white transition-all"><Eye size={14}/></button>
                        <button onClick={() => onView(v.id, 'PKR')} className="p-1.5 bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-500 rounded-lg hover:bg-sky-600 hover:text-white transition-all"><Printer size={14}/></button>
                        <button onClick={() => onView(v.id, 'SAR')} className="p-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-500 rounded-lg hover:bg-emerald-600 hover:text-white transition-all"><ExternalLink size={14}/></button>
                      </div>
                    </td>
